...
hist(
  outliers,
  main = "Blood Pressure Distribution (outliers only)",
  xlab = "BP (mm Hg)",
  ylab = "Number of Patients",
  col = "grey"
)
...
The min and max values represent a ~2.7 sigma interval, which represents 99.3% of the dataset. Outside that range would be only 0.7% of the values, or the outliers.
...
**Chest pain type**:
This is a categorical variable with 4 possible outcomes, as mentioned in the first section. For this variable, I will be using the Multinomial Distribution

**Resting blood pressure in millimeters of mercury (mm Hg)**:
This is a continuous variable, mesuring the blood pressure of patients. In this case, the Normal Distribution is the most appropriate.

**Cholesterol level in mg/dl**:
Same as above, this is continuous, mesuring the cholesterol level in mg/dl.Normal Distribution will be the used one.
...
tab_1 = table(var1)
# parameter
phat1 = tab_1[2] / sum(tab_1)
print(paste0("Since the variables are only 0 and 1, the parameter for this model is ",
             "the chance of getting 1. In this case ", format(phat1, digits = 3, nsmall = 2)))
```
...
var3_mean = mean(var3)
paste0("Mean: ", var3_mean)
var3_sigma = sd(var3)
paste0("SD: ", var3_sigma)
```
...
var4_mean = mean(var4)
paste0("Mean: ", var4_mean)
var4_sigma = sd(var4)
paste0("SD: ", var4_sigma)
```
...
random_set_1 = rbinom(n = 1000, size = 1, prob = phat1)
print(paste0(
  "When summing the sample generated by the binomial distribution, we get ",
  sum(random_set_1),
  ". That means that 1 (male) was", 
  " predicted that amount of times, so it is our prediction."
))
```
...
```{r}
# generate 1000 random samples
sample2 = rmultinom(n = 1000, size = 1, prob = phat2)
# count each category
n0 = sum(sample2[1,])
n1 = sum(sample2[2,])
n2 = sum(sample2[3,])
...
trainset.glm = glm( 
  target ~ sex+cp+thalach+oldpeak+ca+thal,
  trainset,
  family="binomial"
)
```
### c) Specify the significant variables on the output variable at the level of 𝛼=0.05 and explore the related hypotheses test. Estimate the parameters of your model.

```{r}
summary(trainset.glm)
```

For a variable to have a high correlation in predicting an outcome (in this case heart condition), it has to have a P_value <= alpha. In this case, alpha is 0.05.
The columns that show a high correlation in predicting a heart condition are:
...
In the case above, the column "Estimate" are the parameters used to predict the variable "target"
...
```{r}
#since it's a binary outcome, type is response
pred = predict(trainset.glm, testset, type="response")  

len = length(pred) 
pred_values=rep(0, len)  # create vectors of zero
pred_values[pred > 0.5] = 1
table(pred_values, actualvalues = testset[,15])
print(paste0(
  "The model had a predictive accuracy of ",
  mean(pred_values == testset[,15]),  # correctness of prediction,
  "."
))
  
   

print("The functional form can be written as below:")
print(paste0(
  "t = ",
  "sex * ", coef(summary(trainset.glm))[2],
  " + cp * ", coef(summary(trainset.glm))[3],
  " + thalach * ", coef(summary(trainset.glm))[4],
  " + oldpeak * ", coef(summary(trainset.glm))[5],
  " + ca * ", coef(summary(trainset.glm))[6],
  " + thal * ", coef(summary(trainset.glm))[7]
))
```
