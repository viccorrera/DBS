...
print(paste0(
  ...
  sum(random_set_1),
  ". That means that 1 (male) was predicted that amount of times, so it is our prediction."
))
```
...
```{r}
pred = predict(trainset.glm, testset, type="response")  #since it's a binary outcome, type is response

len = length(pred) 
pred_values=rep(0, len)
